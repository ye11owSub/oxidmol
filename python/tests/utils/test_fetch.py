...
@pytest.mark.asyncio
async def test_download_falls_back_to_second_url():
...
@pytest.mark.asyncio
async def test_download_raises_when_all_fail():
...
def test_fetch_many_returns_dict(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    monkeypatch.setattr(fetch_mod, "CACHE_DIR", tmp_path)
